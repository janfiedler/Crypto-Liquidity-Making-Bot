...
process.on('message', async function(data) {
    switch (data.type) {
        case "init":
            config = data.config;
            await itbit.setConfig(data.config);
            init();
            break;
        case "stop":
            stop = true;
...
let init = async function(){
    ...
    await strategy.init(config,myAccount[config.name], db, itbit);
    begin();
};

let getBalance = async function(){
    const rawBalance = await itbit.getBalance();

...
let setFailedSellOrder = function(failedOrder){
    return new Promise(function (resolve) {
        db.run(`UPDATE orders SET status = ?, sell_status = ?, completed_at = ? WHERE buy_id = ? AND (sell_id IS NULL OR sell_id = ?);`, "failed", failedOrder.status, new Date().toISOString(), failedOrder.id, "", function(err) {
            if (err) {
                console.error(err.message);
            } else {
                resolve(true);
